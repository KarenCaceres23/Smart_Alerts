...
class InfluxSensorRepository:
    ...
    def __init__(self, url: str | None = None, token: str | None = None, org: str | None = None, bucket: str | None = None):
    ...
    def get_latest_reading(self, sensor_id: str, zone: str, time_window_minutes: int = 15) -> Optional[SensorReading]:
        """Obtiene la lectura más reciente de un sensor dentro de una ventana de tiempo."""
        if not self.query_api:
            logger.warning(f"Cliente InfluxDB no configurado. Retornando None para sensor {sensor_id}.")
            return None
        ...
        try:
            ...
            if flow_rate is not None and flow_rate < 0: flow_rate = 0.0
            if daily_volume is not None and daily_volume < 0: daily_volume = 0.0
