...
def main():
    ...
    try:
        ...
        if not results:
            logger.info(
                "ℹ️ No hay mensajes nuevos. Por favor, envía un mensaje a tu bot en Telegram y vuelve a correr este script."
            )
            sys.exit(0)
        ...
        for cid, info in chats.items():
            ...
            str_id = str(cid)
            safe_id = (
                str_id[:3] + "*" * (len(str_id) - 5) + str_id[-2:] if len(str_id) > 5 else str_id
            )

            logger.info(
                f"ID: {cid} (Seguro: {safe_id}) | Tipo: {info['type']} | Nombre/User: {info['title']}"
            )
