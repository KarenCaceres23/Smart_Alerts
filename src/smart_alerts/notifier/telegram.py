...
class TelegramNotifier(BaseNotifier):
    ...
    """
    def __init__(self, config: AppConfig, cooldown_manager: CooldownManager, audit_logger: Optional[AuditLogger] = None):
        self.config = config
        ...
        while attempts < self.config.telegram_max_retries:
            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    data = response.json()
                    retry_after = data.get("parameters", {}).get("retry_after", self.config.telegram_backoff_seconds)
                    logger.warning(
                ...
                if data.get("ok"):
                    ...
                    logger.info(
                        ...
                        extra={**log_context, "attempts": attempts, "latency_ms": latency, "resultado": "exito"}
                    )
                    ...
                    logger.error(f"Error lógico de Telegram: {last_error}", extra={**log_context, "attempts": attempts})
                    break # No es recuperable normalmente
                
            except Timeout:
                last_error = "Timeout Error"
                logger.warning(f"Timeout al conectar con Telegram (intento {attempts}/{self.config.telegram_max_retries})", extra=log_context)
                if self.audit_logger:
                    self.audit_logger.log_event(state=AuditState.RETRYING, alert_id=alert.alert_id, rule_id=alert.rule_id, sensor_id=alert.sensor_id, severity=alert.severity, reason=last_error, attempts=attempts)
                time.sleep(self.config.telegram_backoff_seconds)
                
            except ConnectionError:
                last_error = "Connection Error"
                logger.warning(f"Error de conexión (intento {attempts}/{self.config.telegram_max_retries})", extra=log_context)
                if self.audit_logger:
                    self.audit_logger.log_event(state=AuditState.RETRYING, alert_id=alert.alert_id, rule_id=alert.rule_id, sensor_id=alert.sensor_id, severity=alert.severity, reason=last_error, attempts=attempts)
                time.sleep(self.config.telegram_backoff_seconds)
                ...
                last_error = str(e)
                logger.warning(f"Excepción en petición (intento {attempts}): HTTP {last_status_code}", extra=log_context)
                time.sleep(self.config.telegram_backoff_seconds)
